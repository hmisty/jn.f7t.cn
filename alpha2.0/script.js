...
(function(){
    ...
    function createWhitePixels(r, c) {
        return Array(r).fill().map(() => 
            Array(c).fill().map(() => ({ r: 255, g: 255, b: 255 }))
        );
    }
    ...
    newCanvasBtn?.addEventListener('click', () => {
        let r = Math.min(128, Math.max(1, parseInt(rowsInput.value) || DEFAULT_ROWS));
        let c = Math.min(128, Math.max(1, parseInt(colsInput.value) || DEFAULT_COLS));
    ...
    convertBtn?.addEventListener('click', () => {
        ...
        offCanvas.toBlob((blob) => {
            ...
            reader.onloadend = () => {
                ...
                if (withPrefix?.checked) {
                    currentBase64 = prefix + base64;
                } else {
                ...
                if (previewImage) {
                    previewImage.src = offCanvas.toDataURL('image/png');
                }
            };
    ...
    copyBtn?.addEventListener('click', () => {
        if (currentBase64) {
            ...
            if (mintStatus) mintStatus.innerText = '📋 已复制';
        }
    });
